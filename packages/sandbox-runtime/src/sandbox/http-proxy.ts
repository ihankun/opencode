  ...
  server.on('connect', async (req, socket, head) => {
    ...
    try {
      ...
      if (
        ...
        // Newer Node declarations distinguish shared from non-shared Buffer
        // storage for the CONNECT callback. Copy the sniffed bytes back into
        // the non-shared representation expected by that callback.
