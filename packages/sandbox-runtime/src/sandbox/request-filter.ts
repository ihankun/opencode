...
export async function decideAndRespond(
  ...
  if (!BODYLESS_METHODS.has(req.method ?? 'GET')) {
    ...
    const [a, b] = web.tee()
    // Node and DOM model the same runtime Web Stream with slightly different
    // type surfaces in current @types/node. Keep the cast at this API boundary.
    forCallback = a as unknown as ReadableStream<Uint8Array>
