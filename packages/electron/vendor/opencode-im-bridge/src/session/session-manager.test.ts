...
describeOrSkip("session-manager", () => {
  ...
  describe("getOrCreate", () => {
    ...
    it("falls back to POST /session when no sessions exist", async () => {
      ...
      expect(new Headers((createCall![1] as RequestInit).headers).get("x-opencode-directory")).toBe(
        getChannelWorkingDirectory("qq"),
      )
    ...
    it("replaces a legacy mapping from another working directory", async () => {
      const now = Date.now()
      db.exec(`
        CREATE TABLE IF NOT EXISTS feishu_sessions (
          feishu_key TEXT PRIMARY KEY,
      ...
      db.prepare(
        "INSERT INTO feishu_sessions (feishu_key, session_id, agent, created_at, last_active, is_bound) VALUES (?, ?, ?, ?, ?, ?)",
      ).run("legacy-chat", "ses-legacy", DEFAULT_AGENT, now, now, 1)

      mockFetch(async (input, init) => {
        const url = typeof input === "string" ? input : input instanceof URL ? input.toString() : (input as Request).url
        if (url.endsWith("/session/ses-legacy")) {
          return new Response(JSON.stringify({ id: "ses-legacy", directory: "/Users/example" }), { status: 200 })
        }
        if (url.endsWith("/session") && init?.method === "POST") {
          return new Response(JSON.stringify({ id: "ses-channel" }), { status: 200 })
        }
        return new Response("Not found", { status: 404 })
      })
      ...
      expect(await sm.getOrCreate("legacy-chat", undefined, "feishu")).toBe("ses-channel")
      expect(sm.getSession("legacy-chat")?.session_id).toBe("ses-channel")
