...
async function main(): Promise<void> {
  ...
  const embedded = process.env.OPENCODE_IM_BRIDGE_EMBEDDED === "1"

  if (embedded) {
    logger.info("Embedded mode enabled; using the Electron-managed configuration")
  } else if (forceInit) {
  ...
  const config = await loadConfig(process.env.OPENCODE_IM_BRIDGE_CONFIG)

  if ((!config.feishu?.appId || !config.feishu?.appSecret) && (!config.qq?.appId || !config.qq?.secret) && !config.telegram?.botToken && !config.discord?.botToken && !config.wechat?.enabled && (!config.dingtalk?.appKey || !config.dingtalk?.appSecret)) {
    logger.error(
  ...
  const authorization = serverPassword
    ? `Basic ${Buffer.from(`${serverUsername || "opencode"}:${serverPassword}`).toString("base64")}`
    : undefined
  if (authorization) {
    const originalFetch = globalThis.fetch
    globalThis.fetch = (input, init) => {
      const url = input instanceof Request ? input.url : String(input)
      if (!url.startsWith(serverUrl)) return originalFetch(input, init)
