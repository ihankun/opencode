...
async function openExternalUrl(url) {
  if (isTauri()) {
    await __vitePreload(() => import("./index-CTzXdWo9.js"), true ? __vite__mapDeps([0,1,2]) : void 0, import.meta.url).then((mod) => mod.openUrl(url)).catch(() => window.open(url, "_blank", "noopener,noreferrer"));
    return;
  }
  ...
  const handleDetectBinary = async () => {
    ...
    try {
      const { invoke } = await __vitePreload(async () => {
        const { invoke: invoke2 } = await import("./main-CxRsPsI8.js").then((n) => n.aq);
  ...
  const save = async () => {
    ...
    try {
      const { validateAgainstOfficialConfigSchema } = await __vitePreload(async () => {
        const { validateAgainstOfficialConfigSchema: validateAgainstOfficialConfigSchema2 } = await import("./configOfficialValidator-B80ffwuT.js");
