...
export interface HookDefinition { id: string; name: string; event: HookEvent; command: string; enabled: boolean; timeoutSeconds: number }
export interface HookRun { id: string; hookID: string; hookName: string; event: HookEvent; status: 'completed' | 'failed' | 'timed_out'; output: string; startedAt: number; completedAt: number }
