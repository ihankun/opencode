...
export function ServersSettings() {
  ...
  const [runnerProjects, setRunnerProjects] = useState<Record<string, number>>({})
  const {
    servers,
    activeServer,
    addServer,
  ...
  useEffect(() => {
    void Promise.all(servers.map(async server => ({ id: server.id, projects: getHealth(server.id)?.status === 'online' ? await getProjects(undefined, server.id).catch(() => []) : [] }))).then(results => setRunnerProjects(Object.fromEntries(results.map(result => [result.id, result.projects.length]))))
  }, [servers, getHealth])
